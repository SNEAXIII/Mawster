...
class TestChampionSevenStarsEndpoint:
    @pytest.mark.asyncio
    async def test_admin_can_toggle_true_to_false(self, session):
        await push_one_admin()
        champ = await push_champion("Hercules", "Cosmic", is_7_stars_available=True)

        response = await execute_patch_request(
            f"/admin/champions/{champ.id}/seven-stars",
            payload=None,
            headers=ADMIN_HEADERS,
        )

        assert response.status_code == 200
        assert response.json()["is_7_stars_available"] is False

        get_resp = await execute_get_request(f"/champions/{champ.id}", headers=ADMIN_HEADERS)
        assert get_resp.json()["is_7_stars_available"] is False

    @pytest.mark.asyncio
    async def test_admin_can_toggle_false_to_true(self, session):
        await push_one_admin()
        champ = await push_champion("Spider-Man", "Science", is_7_stars_available=False)
        ...
        assert response.status_code == 200
        assert response.json()["is_7_stars_available"] is True

        get_resp = await execute_get_request(f"/champions/{champ.id}", headers=ADMIN_HEADERS)
        assert get_resp.json()["is_7_stars_available"] is True
    ...
    async def test_unauthenticated_is_401(self, session):
        response = await execute_patch_request(
            f"/admin/champions/{uuid.uuid4()}/seven-stars",
            payload=None,
            headers=None,
        )
