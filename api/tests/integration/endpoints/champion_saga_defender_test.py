...
class TestChampionSagaDefenderEndpoint:
    @pytest.mark.asyncio
    async def test_admin_can_toggle_true_to_false(self, session):
        await push_one_admin()
        champ = await push_champion("Storm", "Mutant", is_saga_defender=True)

        response = await execute_patch_request(
            f"/admin/champions/{champ.id}/saga-defender",
            payload=None,
    ...
    async def test_admin_can_toggle_false_to_true(self, session):
        await push_one_admin()
        champ = await push_champion("Magneto", "Mutant", is_saga_defender=False)
    ...
    async def test_non_admin_is_forbidden(self, session):
        await push_one_user()
        champ = await push_champion("Thor", "Cosmic")
    ...
    async def test_unauthenticated_is_401(self, session):
        response = await execute_patch_request(
            f"/admin/champions/{uuid.uuid4()}/saga-defender",
            payload=None,
            headers=None,
        )
