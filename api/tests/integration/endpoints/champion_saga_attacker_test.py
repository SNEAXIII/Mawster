...
class TestChampionSagaAttackerEndpoint:
    @pytest.mark.asyncio
    async def test_admin_can_toggle_true_to_false(self, session):
        await push_one_admin()
        champ = await push_champion("Hercules", "Cosmic", is_saga_attacker=True)

        response = await execute_patch_request(
            f"/admin/champions/{champ.id}/saga-attacker",
            payload=None,
        ...
        assert get_resp.json()["is_saga_attacker"] is False

    @pytest.mark.asyncio
    async def test_admin_can_toggle_false_to_true(self, session):
        await push_one_admin()
        champ = await push_champion("Spider-Man", "Science", is_saga_attacker=False)
        ...
        assert response.status_code == 200
        assert response.json()["is_saga_attacker"] is True
    ...
    async def test_unauthenticated_is_401(self, session):
        response = await execute_patch_request(
            f"/admin/champions/{uuid.uuid4()}/saga-attacker",
            payload=None,
            headers=None,
        )
