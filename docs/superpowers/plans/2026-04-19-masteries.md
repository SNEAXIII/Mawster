...
class TestAdminMasteryCRUD:
    ...
    async def test_create_mastery_ok(self):
        ...
        response = await execute_post_request(
            ADMIN_MASTERIES_URL,
            {"name": "ASSASSIN", "max_value": 5, "order": 0},
    ...
    async def test_update_mastery_ok(self):
        ...
        assert response.json()["max_value"] == 5  # unchanged
        ...
        {masteries.map((mastery) => {
          ...
          return (
            <Card
              ...
              data-cy={`mastery-card-${mastery.mastery_name.toLowerCase()}`}
            >
...
```tsx
{
  vm.activeTab === RosterTab.Mastery && vm.selectedAccountId && (
    <MasteryTab
      ...
      isOwner={
        vm.accounts.find((a) => a.id === vm.selectedAccountId)?.user_id === vm.accounts[0]?.user_id // current user owns their own accounts
      }
