...
| **i18n** | Custom React Context | `front/app/lib/i18n/` — EN (default), FR |
...
### Alliance
`name` (max 100), `tag` (max 10), `description`, `created_at`
→ has many `GameAccount` (members)

### GameAccount
`user_id` (FK → User), `alliance_id` (FK → Alliance, nullable), `game_pseudo` (max 50), `is_primary`, `created_at`
→ belongs to User, Alliance; has many `ChampionUser` (roster)
...
| `/alliances` | alliance_controller | CRUD alliances |
...
- **shadcn/ui**: components in `components/ui/`, imported with `@/components/ui/...`
- **i18n**: `useI18n()` hook returns `{ t, locale, setLocale }` — translations in `app/lib/i18n/{en,fr}.ts`
...
- **No ORM lazy loading**: relationships must be explicitly loaded (async SQLModel limitation)
