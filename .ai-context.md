...
→ has one `owner` (GameAccount), many `members` (GameAccount via alliance_id), many `officers` (AllianceOfficer)

### AllianceOfficer (join table)
...
→ belongs to User, Alliance; has many `ChampionUser` (roster); optional `owned_alliance`, many `officer_entries`
...
| `/alliances` | alliance_controller | CRUD alliances + officer management (`POST/DELETE /{id}/officers`) |
