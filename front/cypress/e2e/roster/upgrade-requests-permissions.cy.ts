...
describe('Roster – Upgrade Requests (Permissions)', () => {
  ...
  it("regular member cannot cancel upgrade requests in another member's roster preview", () => {
    setupAllianceWithMember('ur-t6', 'Black Widow', 'Skill').then(({ ownerData, memberData: member2Data, allianceId, championUserId }) => {
  ...
  it('shows correct count for multiple upgrade requests', () => {
    setupAllianceWithMember('ur-t8', 'HulkMulti', 'Science').then(({ ownerData, memberData, memberAccId, championUserId: cu1Id }) => {
