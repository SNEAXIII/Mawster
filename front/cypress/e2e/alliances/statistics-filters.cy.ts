...
describe('Alliance Statistics – Filters & Champion chart', () => {
  ...
  it('war filter shows only ended wars and filters the chart', () => {
    withWarScenario('stat-wf', 'Ended Enemy').then(({ ownerToken, ownerUserId, allianceId, champId, cuId, warId }) => {
  ...
  it('war filter re-scopes the stats table, not only the chart', () => {
    withTwoEndedWarsTwoPlayers('stat-wft').then(({ ownerUserId, ownerAccId, memberAccId, warOneId, warTwoId }) => {
  ...
  it('season filter switches the table to a past season and rescopes the wars', () => {
    withTwoSeasonsOneWarEach('stat-sf').then(
      ({ ownerUserId, ownerAccId, memberAccId, pastSeasonId, currentSeasonId }) => {
        const ownerRow = `statistics-row-${ownerAccId}`;
        const memberRow = `statistics-row-${memberAccId}`;
  ...
  it('keeps the filter bar reachable when a war filter empties the table', () => {
    withWarScenario('stat-wfe', 'Fought').then(({ ownerToken, ownerUserId, allianceId, champId, cuId, warId }) => {
  ...
  it('clicking a row shows that player name in the chart area', () => {
    withWarScenarioTwoPlayers('stat-pname', 'Enemy').then(
  ...
  it("group filter updates the champion chart to show only that group's champions", () => {
    ...
    withWarScenarioDiffChampsPlayers('stat-grc', 'Enemy', 2).then(
      ({ ownerToken, ownerUserId, allianceId, champ1Id, champ2Id, cuOwnerId, cuMemberId, warId }) => {
  ...
  it('deathless metric shows only ko_count=0 fights by default, all shows both', () => {
    withWarScenarioTwoOwnerChamps('stat-dl', 'Enemy').then(
      ({ ownerToken, ownerUserId, allianceId, champ1Id, champ2Id, cu1Id, cu2Id, warId }) => {
  ...
  it('filters by group — shows G1 player and G2 player in their respective filters', () => {
    // owner → G1 (battlegroup 1), member → G2 (battlegroup 2)
    withWarScenarioDiffChampsPlayers('stat-grp', 'Enemy', 2).then(
      ({
        ownerToken,
        ownerUserId,
        ownerPseudo,
