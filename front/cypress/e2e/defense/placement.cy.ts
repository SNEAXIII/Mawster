  ...
  return setupAdmin(`${tokenPrefix}-admin`).then((adminData) => {
    return setupUser(`${tokenPrefix}-owner`).then((ownerData) => {
      return cy.apiCreateGameAccount(ownerData.access_token, ownerPseudo, true).then((ownerAcc) => {
        ...
        return cy
          .apiCreateAlliance(ownerData.access_token, `${allianceTag}All`, allianceTag, ownerAccId)
          .then((alliance) => {
    ...
    setupDefenseScenario('def-pl-sel-pref', 'SelPrefPlyr', 'SP', [
      {
        ...
        cls: 'Science',
        rarity: '7r3',
        options: { is_preferred_attacker: true },
      },
      {
        name: 'Wolverine',
        cls: 'Mutant',
        rarity: '7r3',
        options: { is_preferred_attacker: false },
      },
    ]).then(({ ownerData }) => {
      cy.uiLogin(ownerData.login);
      cy.navTo('defense');
    ...
    setupDefenseScenario('def-pl-sel-label', 'SelLabelPlyr', 'SL', [
      {
        ...
        cls: 'Cosmic',
        rarity: '7r5',
        options: { ascension: 1, is_ascendable: true },
      },
      { name: 'Wolverine', cls: 'Mutant', rarity: '7r3' },
    ]).then(({ ownerData }) => {
      cy.uiLogin(ownerData.login);
  ...
  it('owner picker shows ascension in rarity label', () => {
    ...
    setupAdmin('def-pl-oasc-admin')
      ...
      .then((memberAcc) => {
        ...
        return cy.apiLoadChampion(adminData.access_token, 'Doctor Doom', 'Mystic', {
          is_ascendable: true,
        });
      })
      .then((champs) => {
