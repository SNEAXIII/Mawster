import { setupAttackerScenario, setupPrefightScenario } from '../../support/e2e';
...
function getAvailableAfterBan(
  ...
  cy.apiRequest(ownerToken, 'POST', `/alliances/${allianceId}/wars`, {
    opponent_name: 'Ban Storm War',
    banned_champion_ids: [bannedChampionId],
  }).then((warRes: { body: { id: string } }) => {
    cy.apiRequest(
      memberToken,
      'GET',
      `/alliances/${allianceId}/wars/${warRes.body.id}/bg/1/available-prefight-attackers`,
...
describe('War Prefight', () => {
  ...
  it('returns 422 when no attacker is assigned on the node', () => {
    setupAttackerScenario('pf3').then(({ adminToken, memberData, memberAccId, allianceId, warId }) => {
      const prefightPath = `/alliances/${allianceId}/wars/${warId}/bg/1/prefight`;
      loadAndAddToRoster(adminToken, memberData.access_token, memberAccId, 'Storm', 'Mutant', ({ cu: stormCu }) => {
        cy.apiRequest(
          memberData.access_token,
          'POST',
          prefightPath,
          { champion_user_id: stormCu.id, target_node_number: 10 },
  ...
  it('returns 409 on duplicate pre-fight assignment', () => {
    setupPrefightScenario('pf4').then(({ memberData, allianceId, warId, championUserId, prefightChampionUserId }) => {
      ...
      cy.apiRequest(memberData.access_token, 'POST', prefightPath, payload).then((res) =>
        expect(res.status).to.eq(201),
      );

      cy.apiRequest(memberData.access_token, 'POST', prefightPath, payload, { failOnStatusCode: false }).then((res) =>
      ...
      ({
        ...
        loadAndAddToRoster(
          ...
          ({ champId, cu }) => {
            ...
            cy.apiRequest(memberData.access_token, 'POST', prefightPath, {
              champion_user_id: prefightChampionUserId,
              target_node_number: 10,
            }).then((res) => expect(res.status).to.eq(201));

            cy.apiRequest(memberData.access_token, 'POST', prefightPath, {
              champion_user_id: prefightChampionUserId,
              target_node_number: 11,
    ...
    setupAttackerScenario('pf6').then(({ adminToken, memberData, allianceId, warId, championUserId, memberAccId }) => {
      ...
      loadAndAddToRoster(adminToken, memberData.access_token, memberAccId, 'Iron Man', 'Tech', ({ cu: ironManCu }) => {
        cy.apiRequest(
          memberData.access_token,
          'POST',
          `/alliances/${allianceId}/wars/${warId}/bg/1/prefight`,
          { champion_user_id: ironManCu.id, target_node_number: 10 },
          { failOnStatusCode: false },
  ...
  it('API — banned champion is excluded from available-prefight-attackers', () => {
    setupPrefightScenario('pf8').then(({ ownerData, memberData, allianceId, warId }) => {
      cy.apiRequest(
        memberData.access_token,
        'GET',
        `/alliances/${allianceId}/wars/${warId}/bg/1/available-prefight-attackers`,
      ).then((res) => {
    ...
    setupAttackerScenario('pf-pref-sel').then(
      ({ adminToken, memberData, memberAccId, allianceId, warId, championUserId }) => {
        ...
        cy.apiLoadChampion(adminToken, 'Storm', 'Mutant').then((champs) => {
          ...
          cy.apiRequest(adminToken, 'PATCH', `/admin/champions/${stormId}/prefight`);
