...
describe('War – Bans', () => {
  ...
  it('war declared via API with bans shows ban badges', () => {
    setupWarOwner('war-bans-api', 'BanOfficer2', 'BanAlliance2', 'BB').then(({ adminData, ownerData, allianceId }) => {
  ...
  it('ban selector filters by champion alias', () => {
    setupWarOwner('war-bans-alias', 'BanAlias', 'BanAliasAlliance', 'BX').then(({ adminData, ownerData }) => {
