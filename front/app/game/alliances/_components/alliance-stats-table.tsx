...
function StatCell({
  ...
  return (
    <TableCell
      className={`py-1.5 text-right ${className ?? ''}`}
      data-cy={dataCy}
    >
  ...
  const colRanks = useMemo(() => {
    ...
    return {
      total_fights_weighted: top3(
        rows.map((r) => r.total_fights_weighted),
        true
      ),
      total_kos: top3(
        rows.map((r) => r.total_kos),
        true
      ),
      total_miniboss: top3(
        rows.map((r) => r.total_miniboss),
        true
      ),
      total_boss: top3(
        rows.map((r) => r.total_boss),
        true
      ),
      total_not_fought: top3(
        rows.map((r) => r.total_not_fought),
        true
      ),
      ratio: top3(
        rows.map((r) => r.ratio),
      ...
      wars_participated: top3(
        rows.map((r) => r.wars_participated),
        true
      ),
      avg_fights_per_war: top3(
        rows.map((r) => r.avg_fights_per_war),
        true
      ),
      avg_boss_miniboss_per_war: top3(
        rows.map((r) => r.avg_boss_miniboss_per_war),
        true
      ),
      total_assists: top3(
        rows.map((r) => r.total_assists),
        true
      ),
    }
  }, [rows])
          ...
          return (
            ...
            >
              <TableCell className='py-1.5 font-medium'>
                <span
                  className={`flex items-center gap-1 ${!row.is_current_member ? 'text-muted-foreground' : ''}`}
                >
                  {row.game_pseudo}
                  {!row.is_current_member && (
                    <LogOut
                      data-cy={`former-badge-${row.id}`}
