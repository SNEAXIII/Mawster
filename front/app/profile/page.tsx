...
export default function ProfilePage() {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <InfoRow icon={<LuUser className="h-4 w-4" />} label={t.profile.username} value={user?.name} fallback={t.common.notAvailable} dataCy="username-row" />
            <InfoRow icon={<LuMail className="h-4 w-4" />} label={t.profile.email} value={user?.email} fallback={t.common.notAvailable} dataCy="email-row" />
            <InfoRow icon={<FaDiscord className="h-4 w-4" />} label={t.profile.discordId} value={user?.discord_id} fallback={t.common.notAvailable} dataCy="discord-id-row" />
            <InfoRow icon={<LuCalendar className="h-4 w-4" />} label={t.profile.memberSince} value={user?.created_at ? formatDateLong(user.created_at, locale) : t.common.notAvailable} fallback={t.common.notAvailable} dataCy="member-since-row" />
          </div>
        </CardContent>
      </Card>
      ...
      {/* Bouton deconnexion */}
      <Button variant="outline" className="w-full" data-cy="sign-out-btn" onClick={handleSignOut}>
...
function InfoRow({
  ...
  return (
    <div className="flex items-start gap-3 p-3 rounded-lg bg-gray-50" data-cy={dataCy}>
